...
instance.getUserStats = async (userId) => {
    const totalResult = (await instance.query(
      'select count(fileId) as count, sum(size) as size from file where userId=?',
      [userId]
    ))[0];

    const lastUpload = (await instance.query(
      'select * from file where userId=? order by creationDate desc limit 1',
      [userId]
    ))[0];
    ...
    return {
        ...
        lastUpload: lastUpload != null ? lastUpload.creationDate : null
    };
};

instance.getFilesByUser = async (userId, page, pageSize) => {
    return (await instance.query(
      'select * from file where userId=? order by creationDate desc limit ?, ?',
...
instance.updateUserPassword = async (id, password) => {
    await instance.query('update user set password=? where userId=?', [password, id]);
};
...
instance.deleteFileByFileId = async (fileId) => {
    await instance.query('update file set deletionDate=?, size=0 where fileId=?', [instance.date(Date.now()), fileId]);
};
